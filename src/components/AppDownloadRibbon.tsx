...
export function AppDownloadRibbon() {
  ...
  function dismiss(event: MouseEvent<HTMLButtonElement>) {
    event.currentTarget.closest("aside")?.setAttribute("hidden", "");
    document.documentElement.classList.add("hb-app-ribbon-dismissed");
    try {
    ...
    <>
      <script
        dangerouslySetInnerHTML={{
          __html: `try{if(localStorage.getItem('${DISMISSED_KEY}')==='true')document.documentElement.classList.add('hb-app-ribbon-dismissed')}catch(e){}`,
        }}
      />
      <style>{`.hb-app-ribbon-dismissed #app-download-ribbon{display:none}`}</style>
      <aside
